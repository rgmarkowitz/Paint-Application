...
type gctx = {
  ...
  thickness: bool; (* the current shape thickness *)
  thicknesslevel : int
  }
...
let set_graphics_state (gc: gctx) : unit =
  ...
  if gc.thickness then Graphics.set_line_width gc.thicknesslevel
...
(* this function assigns thickness to gctx *)
let with_thickness (g: gctx) (t: bool) : gctx =
  { g with thickness = t }
  
(*  add for thickness level   *)
let with_thickness_level (g: gctx) (t: int) : gctx = 
  { g with thicknesslevel = t }
