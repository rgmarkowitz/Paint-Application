...
(** A checkbox is a controller for a boolean value associated with a widget.
   ...
   application. All of the added listeners are run whenever this value is changed. 
...
let make_controller (v: 'a) : 'a value_controller =
  let control_list = { contents = [] } in  
  let v_ref = { contents = v } in 
  { add_change_listener = (fun f -> control_list.contents <- 
                          (f :: control_list.contents));
    get_value = (fun () -> v_ref.contents);
    
    change_value = fun f -> v_ref.contents <- f;
                   List.iter (fun f -> f v_ref.contents) control_list.contents }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  (* make dimension and checkbox variable to call size of box *)
  let dimension : (int * int) = (20, 20) in 
  (*  make value controller  *)
  let value_controller = make_controller init in 
  (*  make widget  *)
  let checkbox_box = { repaint = (fun (g: Gctx.gctx) -> 
                                   Gctx.draw_string g (22, 0) s;
                                   if not(value_controller.get_value())
                                   then Gctx.draw_rect g (0,20) dimension
                                   else Gctx.fill_rect g (0,20) dimension);
                                 
                       handle = (fun (g: Gctx.gctx) (e: Gctx.event) ->
                                (begin match Gctx.event_type e with 
                                  |Gctx.MouseDown ->
                                    if event_within g e dimension 
                                    then value_controller.change_value 
                                       (not(value_controller.get_value()))
                                    else ()          
                                  |_ -> ()
                                end));
                       size = fun () -> dimension } in  
  (checkbox_box, value_controller)
...
let slider (value: int) (s: string) : widget * int value_controller = 
  (*  make value_controller  *)
  let value_controller = make_controller value in 
  (*  make slider widget  *)
  let slider_widg = { repaint = (fun (g: Gctx.gctx) -> 
                                Gctx.draw_string g (55, 0) s;
                                Gctx.draw_rect g (0, 20) (50, 20);
                                Gctx.fill_rect g 
                                  (0, 20) (value_controller.get_value(), 20));
                      handle = (fun (g: Gctx.gctx) (e: Gctx.event) -> 
                                let (x, y) = Gctx.event_pos e g in
                                (begin match Gctx.event_type e with 
                                  |Gctx.MouseDown ->
                                     value_controller.change_value x
                                  |Gctx.MouseDrag ->
                                    value_controller.change_value
                                      (min x 50)        
                                  |Gctx.MouseUp ->
                                    value_controller.change_value x  
                                  |_ -> ()
